...
struct Sphere {
	...
	bool contains(const Array<T, 3> & p, const Arithmetic & a) const
	{
...
struct Rect {
	...
	bool contains(const Array<T, 3> & p) const
	{
	...
	bool contains_or_on_boundary(const Array<T, 3> & p) const
	{
		return (p[0] >= x0) && (p[0] <= x1) &&
			   (p[1] >= y0) && (p[1] <= y1) &&
			   (p[2] >= z0) && (p[2] <= z1);
	...
	template<class Arithmetic>
	bool contains_or_on_boundary(const Array<T, 3> & p, const Arithmetic & a) const
	{
		return std::abs(a.dist_x(0.5*(x0+x1), p[0])) <= 0.5*(x1-x0) &&
				std::abs(a.dist_y(0.5*(y0+y1), p[1])) <= 0.5*(y1-y0) &&
				std::abs(a.dist_z(0.5*(z0+z1), p[2])) <= 0.5*(z1-z0);
	...
	Rect merge(const Rect & r2) const
	{
		Rect ret;
		ret.x0 = std::min(x0, r2.x0);
		ret.x1 = std::max(x1, r2.x1);
		ret.y0 = std::min(y0, r2.y0);
		ret.y1 = std::max(y1, r2.y1);
		ret.z0 = std::min(z0, r2.z0);
		ret.z1 = std::max(z1, r2.z1);
		return ret;
	}
...
template<class T>
std::ostream & operator<<(std::ostream & s, const Rect<T> & box)
{
