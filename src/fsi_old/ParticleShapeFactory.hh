...
void ParticleShapeLibrary<T>::read_and_store_mesh(const std::string & file_name, const std::string & tag)
{
	...
	ParticleShape<T> * shape = new ParticleShape<T>(vertices, indices, num_nodes, num_els);
...
{
	PLB_PRECONDITION(id < id_shape_map.size())
...
{
	PLB_PRECONDITION(id_shape_map.find(id) != id_shape_map.end())
	return id_shape_map[id];
}
